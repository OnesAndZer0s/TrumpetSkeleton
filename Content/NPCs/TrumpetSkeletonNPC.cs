...
using TrumpetSkeleton.Common.Configs;
using TrumpetSkeleton.Content.Items;

namespace TrumpetSkeleton.Content.NPCs
{
	...
	public class TrumpetSkeletonNPC : ModNPC
	{
		...
		public override void SetStaticDefaults() {
			...
			NPCID.Sets.NPCBestiaryDrawModifiers value = new NPCID.Sets.NPCBestiaryDrawModifiers(0) {
				...
				Velocity = 1f // Draws the NPC in the bestiary as if its walking +1 tiles in the x NPC.direction

				// @TODO add sporratic sounds
			};
		...
		public override void SetDefaults() {
			...
			AIType = NPCID.BoneThrowingSkeleton; // Use vanilla zombie's type when executing AI code. (This also means it will try to despawn during daytime)
			AnimationType = NPCID.BoneThrowingSkeleton; // Use vanilla zombie's type when executing animation code. Important to also match Main.NPCFrameCount[NPC.type] in SetStaticDefaults.
			Banner = NPC.type; // Makes this NPC get affected by the normal zombie banner.
			BannerItem = ModContent.ItemType<Items.Placeable.TrumpetSkeletonBannerItem>(); // Makes kills of this NPC go towards dropping the banner it's associated with.
		...
		public override void SetBestiary(BestiaryDatabase database, BestiaryEntry bestiaryEntry) {
			...
			bestiaryEntry.Info.AddRange(new IBestiaryInfoElement[] {
				...
				new FlavorTextBestiaryInfoElement("I'd love to write something down, but the sounds got so obnoxious."),
			});
		}
		...
		public override void AI() {
			...
			if (NPC.ai[3] < (float)numIs60 && (Main.eclipse || !Main.dayTime || (double)NPC.position.Y > Main.worldSurface * 16.0)) {
				if ((NPC.type >= NPCID.BoneThrowingSkeleton && NPC.type <= NPCID.BoneThrowingSkeleton4 || true) && Main.rand.NextBool(800)) {
					// SoundEngine.PlaySound(Trumpet.DootSound, NPC.position);
				}
				NPC.TargetClosest();
			}
			else if (!(NPC.ai[2] > 0f)) {
			...
			if ((NPC.type >= NPCID.BoneThrowingSkeleton && NPC.type <= NPCID.BoneThrowingSkeleton4 || true)) {
				...
				if (NPC.confused) {
					...
					if (NPC.localAI[0] == 0) {
						...
						NPC.localAI[0] = Main.rand.Next(256, 512);
					}
					...
					if (NPC.ai[2] > 0f) {
						...
						if (NPC.ai[1] == (float)numIs35) {
							NPC.netUpdate = true;
							if (!Main.dedServ && NPC.active) {
								SoundEngine.PlaySound(Trumpet.DootSound, NPC.position);

								for (int i = 0; i < Main.maxProjectiles; i++) {
									Projectile proj = Main.projectile[i];
									float distance = (NPC.position.DistanceSQ(proj.position) / 128);
									if (!proj.minion && proj.active && proj.position.WithinRange(NPC.position, 256) && Math.Sign(proj.position.DirectionTo(NPC.position).X) == Math.Sign(-NPC.direction)) {
										proj.velocity.X += (proj.velocity.X / Main.rand.NextFloat(10, 50)) + (128 / distance) * NPC.direction;
										proj.friendly = false;
									}
								}
								...
								for (int i = 0; i < Main.player.Length; i++) {
									...
									float distance = (NPC.position.DistanceSQ(player.position) / 128);
									if (player.active && player.position.WithinRange(NPC.position, 256) &&
									Math.Sign(player.position.DirectionTo(NPC.position).X) ==
									Math.Sign(-NPC.direction)) {
										player.velocity.X += (player.velocity.X / Main.rand.NextFloat(8, 10)) + (256 / distance) * NPC.direction;
										player.Hurt(PlayerDeathReason.ByCustomReason(player.name + " was too spooked"), NPC.damage, NPC.direction, false, true, false);
						...
						if (flag20)//about to attack
						{
							...
							if (numSqrtPosX2PlusPosY2 < numIs200f) {
								...
								NPC.ai[2] = 3f;
								NPC.ai[1] = Main.rand.Next(70, 140);
		...
		public override void OnKill() {
			...
			if (Main.rand.Next(ModContent.GetInstance<TrumpetSkeletonConfig>().ChanceToDropTrumpet) == 0) {
				Item.NewItem(NPC.GetSource_Death(), NPC.Center, ModContent.ItemType<Trumpet>(), 1, true);
			}
		}

		public override float SpawnChance(NPCSpawnInfo spawnInfo) {
			return (SpawnCondition.Dungeon.Active) ? ModContent.GetInstance<TrumpetSkeletonConfig>().TrumpetSkeletonSpawnChance : 0f;
		}
	}
}